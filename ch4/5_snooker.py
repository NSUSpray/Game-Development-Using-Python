...
class Ball:
    ...
    def step(self, dt: float) -> None:
        self.pos = [x + dx*dt for x, dx in zip(self.pos, self.vel)]
        self.vel = [dx * DECELERATION**dt for dx in self.vel]
        if abs(sum([dx*dx for dx in self.vel])) < self.r*2:
            self.vel = [0.0, 0.0]
...
while True:
    for e in pg.event.get():
        match e.type:
            ...
            case pg.MOUSEBUTTONUP:
                cue.vel = [(ex - cx)*2 for ex, cx in zip(e.pos, cue.pos)]
    bat_pos = pg.mouse.get_pos() if pg.mouse.get_pressed()[0] else None
    dt = DT
    while True:
    ...
    if bat_pos:
        pg.draw.line(display, (255, 255, 127), cue.pos, bat_pos, 5)
